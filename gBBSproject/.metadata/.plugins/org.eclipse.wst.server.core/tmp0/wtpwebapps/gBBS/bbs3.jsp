...
<body style = "background-color:#F0FFFF;">
		...
		<div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarColor02">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="bbs3.jsp">공지사항</a></li>
                </ul>
            </div>
		</div>
	</nav>
	...
	<div class="container" style="margin-top: 10px;">
		<div class="row">
			<div style="font-family: 'Jua', sans-serif; font-size: 30px;">공지사항</div>
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd;">
				...
				<tbody>
					<% 
					   ...
					   int board_Number = 3;
					   ArrayList<Bbs> list3 = bbsDAO.getList(board_Number, pageNumber);
					   for(int i=0; i<list3.size(); i++){
					%>
					<tr>
						<td><%= list3.get(i).getPost_Number() %></td>
						<td><%= list3.get(i).getPost_Category() %></td>
						<td>
						...
						<td><%=list3.get(i).getUser_NickName()%></td>
						<td><%= list3.get(i).getPost_Recommend() %></td>
						<td><%= list3.get(i).getPost_Views() %></td>
						<td><%= list3.get(i).getPost_InputDate().substring(0,11) + " " + list3.get(i).getPost_InputDate().substring(11,13)+"시 " + list3.get(i).getPost_InputDate().substring(14,16) + "분"%></td>
					</tr>
           ...
           %>
			<div>
				<div>
					<a href="bbs3.jsp?pageNumber=<%=pageNumber-1%>" class="btn btn-primary" style="float:left; margin-right:2px;">이전</a>
				</div>
				<%   
                } if(bbsDAO.nextPage(pageNumber + 1)){
           %>
				<div>
					<a href="bbs3.jsp?pageNumber=<%=pageNumber+1%>" class="btn btn-primary" style="float: left;">다음</a>
				</div>
			</div>
			<%   
                }
